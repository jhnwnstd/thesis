...
def build_kenlm_model(
    corpus_name: str, q: int, corpus_path: Path, model_directory: Path
...
class CorpusManager:
    ...
    corpora_tokens: List[str] = []  # List to store all words across corpora
    ...
    def __init__(
        ...
        self.corpus: Counter[str] = Counter()
        self.load_corpus()
        self.train_set: List[str] = []
        self.test_set: List[Tuple[str, str, str]] = []
        self.model: Dict[int, kenlm.Model] = {}
    ...
    def clean_text(self, text: str) -> List[str]:
        ...
        return [
            ...
            if len(part) >= self.config.min_word_length
        ]
        ...
        if file_path.is_file():
            ...
            try:
                ...
                self.corpus.update(
                    self.clean_text(
                        " ".join(
                            getattr(nltk.corpus, nltk_corpus_name).words()
                        )
                    )
                )
            except AttributeError:
                raise ValueError(
                    f"File '{file_path}' does not exist and NLTK corpus '{nltk_corpus_name}' not found."
                )
            except Exception as e:
                raise RuntimeError(
                    f"Failed to load corpus '{self.corpus_name}': {e}"
                )
    ...
    def prepare_datasets(self):
        ...
        self.all_words.update(
            [original_word for _, _, original_word in self.test_set]
        )

        # Generate the formatted training list path and the corresponding models
        formatted_train_set_path = (
            self.config.sets_dir
            / f"{self.corpus_name}_formatted_train_set.txt"
        )
        self.generate_formatted_corpus(
            self.train_set, formatted_train_set_path
        )
    ...
    def _split_hapax(self):
        ...
        hapax_legomena = {
            word for word, count in self.corpus.items() if count == 1
        }
        ...
        for word, count in self.corpus.items():
            if word in hapax_legomena:
                ...
                for _ in range(
                    count
                ):  # Handle the case where a hapax legomenon appears more than once due to preprocessing
                    modified_word, missing_letter, _ = (
                        self.replace_random_letter(word, used_combinations)
                    )
        ...
        formatted_train_set_path = (
            self.config.sets_dir
            / f"{self.corpus_name}_formatted_train_set_hapax.txt"
        )
        self.generate_formatted_corpus(
            self.train_set, formatted_train_set_path
        )
        ...
        if self.debug:
            logging.info(
                f"Identified {len(hapax_legomena)} unique hapax legomena for testing."
            )
    ...
    def _split_type_a(self):
        ...
        for word in testing_word_types:
            ...
            for _ in range(word_count):
                modified_word, missing_letter, _ = self.replace_random_letter(
                    word, used_combinations
                )
                self.test_set.append((modified_word, missing_letter, word))
                used_combinations.add((word, missing_letter))
        ...
        if self.debug:
            self.save_list_to_file(
                self.train_set, f"{self.corpus_name}_train_list_a.txt"
            )
            self.save_list_to_file(
                self.test_set, f"{self.corpus_name}_test_list_a.txt"
            )
    ...
    def _split_type_b(self):
        ...
        if self.debug:
            self.save_list_to_file(
                self.train_set, f"{self.corpus_name}_train_list_b.txt"
            )
            self.save_list_to_file(
                self.test_set, f"{self.corpus_name}_test_list_b.txt"
            )

    def generate_formatted_corpus(
        self, data_set: List[str], formatted_corpus_path: Path
    ) -> Path:
        """
    ...
    def generate_models_from_corpus(self, corpus_path: Path):
        ...
        for q in self.config.q_range:
            if q not in self.model:
                ...
                _, binary_file = build_kenlm_model(
                    self.corpus_name, q, corpus_path, model_directory
                )
                if binary_file:
    ...
    def generate_and_load_models(self):
        ...
        for q in self.config.q_range:
            if q not in self.model:
                formatted_train_set_path = (
                    self.config.sets_dir
                    / f"{self.corpus_name}_formatted_train_list.txt"
                )
                self.generate_formatted_corpus(
                    self.train_set, formatted_train_set_path
                )
    ...
    def replace_random_letter(
        self, word: str, used_combinations: set
    ) -> Tuple[str, str, str]:
        """
        ...
        vowel_indices = [
            ...
            if letter in Letters.VOWELS.value
        ]
        consonant_indices = [
            i
            for i, letter in enumerate(word)
            if letter in Letters.CONSONANTS.value
        ]
        ...
        valid_vowel_indices = [
            i for i in vowel_indices if (word, i) not in used_combinations
        ]
        valid_consonant_indices = [
            i for i in consonant_indices if (word, i) not in used_combinations
        ]

        # Choose from the valid indices based on the vowel replacement ratio
        letter_indices = (
            valid_vowel_indices
            if self.rng.random() < self.config.vowel_replacement_ratio
            and valid_vowel_indices
            else valid_consonant_indices
        )
        if not letter_indices:
            letter_indices = (
                valid_vowel_indices or vowel_indices
            )  # Fallback if no valid consonant indices
    ...
    def save_list_to_file(self, data_list: List, file_name: str):
        ...
        with file_path.open(
            "w", encoding="utf-8", buffering=8192
        ) as file:  # 8192 bytes buffer size
            file.write(aggregated_data_str)


def analyze_test_tokens_not_in_training_performance(
    corpus_manager: CorpusManager, test_tokens: set
) -> int:
    """
    ...
    if num_analyzed_tokens == 0:
        logging.info(
            "Evaluation: No test tokens found in predictions for analysis."
        )
        return num_analyzed_tokens

    # Initialize an evaluation model with the filtered test set
    temp_eval_model = EvaluateModel(
        corpus_manager, log_initialization_details=False
    )
    temp_eval_model.test_set = filtered_test_set

    # Evaluate the model's predictions on the filtered test set
    evaluation_metrics, _ = temp_eval_model.evaluate_character_predictions(
        temp_eval_model.prediction_method
    )

    logging.info(
        f"Evaluated {num_analyzed_tokens} tokens not found in training data:"
    )
    ...
    for i in range(1, 4):
        ...
        logging.info(
            f"TOP{i} ACCURACY: {accuracy:.2%} | TOP{i} VALIDITY: {validity:.2%}"
        )
...
def log_evaluation_results(
    evaluation_metrics: dict, corpus_name: str, prediction_method_name: str
):
    """
...
def run(corpus_name: str, config: Config, split_type: str):
    ...
    logging.info(
        f"Processing {formatted_corpus_name} Corpus with split type {split_type}"
    )
    ...
    prediction_method = getattr(
        eval_model.predictor, config.prediction_method_name
    )

    # Evaluate character predictions
    evaluation_metrics, predictions = (
        eval_model.evaluate_character_predictions(prediction_method)
    )
    log_evaluation_results(
        evaluation_metrics, corpus_name, prediction_method.__name__
    )

    # Analyze performance on unique test tokens for split type B
    if split_type == "B":
        analyze_test_tokens_not_in_training_performance(
            corpus_manager, corpus_manager.unique_test_tokens
        )

    # Export prediction details and save summary statistics
    eval_model.export_prediction_details_to_csv(
        predictions, prediction_method.__name__
    )
    eval_model.save_summary_stats_txt(
        evaluation_metrics, predictions, prediction_method.__name__
    )
...
def main():
    ...
    corpora = [
        "cmudict",
        "brown",
        "all_tokens_clmet.txt",
        "all_tokens_lampeter.txt",
