...
class Config:
    ...
    vowel_replacement_ratio: float = (
        0.2  # must sum to 1 with consonant_replacement_ratio
    )
    consonant_replacement_ratio: float = (
        0.8  # must sum to 1 with vowel_replacement_ratio
    )
    min_word_length: int = 4  # minimum word length for evaluation
    prediction_method_name: str = (
        "context_sensitive"  # method to use for prediction
    )
    num_replacements: int = (
        1  # number of replacements to make: Note not all features of this code work with num_replacements > 1
    )
...
def process_corpus(corpus_name: str, config: Config) -> None:
    ...
    try:
        ...
        prediction_method: Callable = getattr(
        ...
        logging.error(
            f"Error processing corpus {corpus_name}: {e}", exc_info=True
        )
...
def main() -> None:
    ...
    corpora: List[str] = [
        "brown",
        "cmudict",
        "sorted_tokens_lampeter.txt",
        "sorted_tokens_openEdges.txt",
        "sorted_tokens_clmet.txt",
    ]
