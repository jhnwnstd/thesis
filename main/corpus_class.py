...
def build_kenlm_model(
    corpus_name, q, corpus_path, model_directory
) -> tuple[int, str | None]:
    """
...
class CorpusManager:
    ...
    unique_words_all_corpora: set[str] = set()
    ...
    def __init__(
        self, corpus_name, config, debug=False
    ):  # Change to True if you want the full test and train set list saved
        """
    ...
    def load_corpus(self) -> list[str]:
        ...
        if file_path.is_file():
            ...
            with file_path.open(
                "r", encoding="utf-8", buffering=1
            ) as file:  # Use buffered reading
            ...
            try:
                ...
                self.corpus = self.clean_text(
                    " ".join(getattr(nltk.corpus, nltk_corpus_name).words())
                )
    ...
    def _shuffle_and_split_corpus(self) -> tuple[list[str], list[str]]:
        ...
        """
        shuffled_corpus = list(
            self.corpus
        )  # Convert corpus to list for shuffling
        self.rng.shuffle(
            shuffled_corpus
    ...
    def prepare_datasets(self):
        ...
        """
        self.train_set, unprocessed_test_set = (
            self._shuffle_and_split_corpus()
        ...
        )  # Generate formatted corpus file

        formatted_test_set = []
        for word in unprocessed_test_set:
            num_replacements = min(self.config.num_replacements, len(word))
            modified_word, missing_letters = self._replace_letters(
                word, num_replacements
            )
            if missing_letters:
                formatted_test_set.append(
                    (modified_word, tuple(missing_letters), word)
                )

        self.test_set = (
            formatted_test_set  # Leave as list for deterministic order
        )
        self.all_words = set(self.train_set).union(
            {original_word for _, _, original_word in self.test_set}
        )

        if self.debug:
            self.save_set_to_file(
                self.train_set, f"{self.corpus_name}_train_set.txt"
            )
            self.save_set_to_file(
                self.test_set, f"{self.corpus_name}_formatted_test_set.txt"
            )
            self.save_set_to_file(
                self.all_words, f"{self.corpus_name}_all_words.txt"
            )

    def generate_formatted_corpus(
        self, data_set, formatted_corpus_path
    ) -> Path:
        """
        ...
        )  # Format words for KenLM compatibility
        with formatted_corpus_path.open(
            "w", encoding="utf-8", buffering=1
        ) as f:  # Use buffered writing
        ...
        for q in self.config.q_range:
            if q not in self.model:
                ...
                if binary_file:
                    ...
                    logging.info(
                        f"Model for {q}-gram loaded from {self.corpus_name}"
                    )
    ...
    def _replace_letters(
        ...
        for _ in range(num_replacements):
            if self.has_replaceable_letter(modified_word):
                modified_word, missing_letter = self._replace_random_letter(
                    modified_word
                )
    ...
    def has_replaceable_letter(self, word) -> bool:
        ...
        """
        return any(Letters.is_vowel(letter) for letter in word) or any(
            Letters.is_consonant(letter) for letter in word
        )
    ...
    def _replace_random_letter(self, word) -> tuple[str, str]:
        ...
        """
        vowel_indices = [
            i for i, letter in enumerate(word) if Letters.is_vowel(letter)
        ]
        consonant_indices = [
            i for i, letter in enumerate(word) if Letters.is_consonant(letter)
        ]
        ...
        letter_indices = (
            ...
            and vowel_indices
            else consonant_indices or vowel_indices
        )
