...
def load_dataset(args: Tuple[str, Path]) -> Tuple[str, Optional[pd.DataFrame]]:
    ...
    try:
        ...
        df = pd.read_csv(
            path,
            usecols=["Top1_Confidence", "Top1_Is_Valid", "Top1_Is_Accurate"],
        )
        return name, df
...
def calculate_histogram_data(
    df: pd.DataFrame, column_name: str, bins: np.ndarray
    ...
    valid_proportions = np.divide(
        valid_counts, total_counts, where=total_counts != 0
    )
    invalid_proportions = np.divide(
        invalid_counts, total_counts, where=total_counts != 0
    )
...
def plot_histogram(
    ...
    Args:
        ...
        valid_color: The color for valid predictions.
    ...
    bins = np.linspace(0.1, 1, num_bins + 1)
    # Calculate valid and invalid proportions
    valid_proportions, invalid_proportions = calculate_histogram_data(
        dataset, column_name, bins
    )
    ...
    if len(threshold_bin_indices) > 0:
        ...
        ax.axvline(float(first_threshold_bin), color="black", linestyle="--")
        ax.annotate(
            f"{first_threshold_bin:.2f}",
            xy=(
                float(first_threshold_bin),
                float(valid_proportions[first_threshold_bin_index]),
            ),
            xytext=(
                float(first_threshold_bin) + 0.05,
                float(valid_proportions[first_threshold_bin_index]) - 0.05,
            ),
        ...
        ax.plot(
            ...
            color="black",
            linestyle="--",
            label=f"Threshold at {threshold*100:.0f}%",
        )
    ...
    label_invalid = (
        "Invalid" if column_name == "Top1_Is_Valid" else "Inaccurate"
    )
    ...
    ax.set_title(
        f"{label} Dataset ({label_valid})", fontsize=16, fontweight="bold"
    )
...
def create_figures_for_datasets(
    loaded_datasets: Dict[str, Optional[pd.DataFrame]], column_name: str
) -> List[Figure]:
    """
    ...
    for (label, dataset), color, ax in zip(
        loaded_datasets.items(), colors, axs.flatten()
    ):
...
def save_figures(figures: List[Figure], filename: str) -> None:
    """
...
def main():
    ...
    for column_name, filename in [
        ("Top1_Is_Accurate", "normalized_accurate_stacked_histograms"),
