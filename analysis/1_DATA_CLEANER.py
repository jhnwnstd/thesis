...
def drop_null_values(df, column):
    ...
    if dropped_rows > 0:
        logger.info(
            f"Dropped {dropped_rows} rows with null '{column}'. New row count: {len(df)}"
        )
    return df
...
def remove_duplicates(df):
    ...
    if duplicates_removed > 0:
        logger.info(
            f"Removed {duplicates_removed} duplicate rows. New row count: {len(df)}"
        )
    return df
...
def clean_dataset(file_path):
    ...
    try:
        ...
        boolean_columns = (
            ["In_Training_Set"]
            + [f"Top{i}_Is_Valid" for i in range(1, 4)]
        ...
        df = validate_predicted_letters(
            df, [f"Top{i}_Predicted_Letter" for i in range(1, 4)]
        )
        ...
        logger.error(
            f"Error: Missing expected column in {file_path.name}: {e}"
        )
    except ValueError as e:
        logger.error(
            f"Error: Data type conversion issue in {file_path.name}: {e}"
        )
