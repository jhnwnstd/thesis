...
def filter_mispredictions(data: pd.DataFrame) -> pd.DataFrame:
    ...
    return data[data['Correct_Letters'] != data['Top1_Predicted_Letter']] if not data.empty else pd.DataFrame()
...
def calculate_confusion_matrix(mispredictions: pd.DataFrame) -> pd.DataFrame:
    ...
    confusion_matrix = pd.crosstab(mispredictions['Correct_Letters'], mispredictions['Top1_Predicted_Letter'])
