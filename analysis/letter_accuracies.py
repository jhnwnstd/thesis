...
def load_dataset(args: Tuple[str, Path]) -> Tuple[str, Optional[pd.DataFrame]]:
    ...
    try:
        ...
        df = pd.read_csv(
            path, usecols=["Top1_Predicted_Letter", "Top1_Is_Accurate"]
        )
        return name, df
...
def calculate_letter_accuracies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    accuracies = (
        df.groupby("Top1_Predicted_Letter")["Top1_Is_Accurate"]
        .agg(["mean", "count", "std"])
        .reset_index()
    )
...
def aggregate_accuracies(
    accuracy_dict: Dict[str, pd.DataFrame],
) -> pd.DataFrame:
    """Aggregates accuracies from multiple datasets."""
    all_letters: set[str] = set()
    indexed: Dict[str, pd.DataFrame] = {}
    for dataset_name, df in accuracy_dict.items():
        if df is not None:
            all_letters.update(df["Letter"])
            indexed[dataset_name] = df.set_index("Letter")

    aggregated_data = []
    for letter in all_letters:
        letter_data: dict = {"Letter": letter}
        for dataset_name, df_indexed in indexed.items():
            if letter in df_indexed.index:
                letter_data[f"{dataset_name}_Accuracy"] = df_indexed.at[
                    letter, "Accuracy"
                ]
                letter_data[f"{dataset_name}_Count"] = df_indexed.at[
                    letter, "Count"
                ]
...
def plot_accuracies(
    accuracy_df: pd.DataFrame, title: str, output_path: Path
) -> None:
...
def plot_aggregated_accuracies(
    aggregated_accuracies: pd.DataFrame, output_path: Path
) -> None:
    ...
    melted_df = pd.melt(
        ...
        value_vars=[
            col
            for col in aggregated_accuracies.columns
    ...
    plt.title(
        "Aggregated Accuracy of Letters Across Datasets",
        fontsize=20,
        fontweight="bold",
    )
    plt.xlabel("Letter", fontsize=16)
...
def perform_anova_on_accuracies(aggregated_accuracies: pd.DataFrame) -> None:
    ...
    accuracy_data = [
        aggregated_accuracies[col].dropna() for col in accuracy_columns
    ]

    f_statistic, p_value = f_oneway(*accuracy_data)
    logger.info(
        f"ANOVA results: F-statistic = {f_statistic}, p-value = {p_value}"
    )

    if p_value < 0.05:
        logger.info(
            "The differences in accuracy scores across datasets are statistically significant."
        )
    else:
        logger.info(
            "The differences in accuracy scores across datasets are not statistically significant."
        )
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        accuracy_dict = dict(
            executor.map(run_accuracy_analysis, DATASET_PATHS.items())
        )
    ...
    for name, accuracy_df in accuracy_dict.items():
        if accuracy_df is not None:
            logger.info(
                f"Letter accuracy for {name} dataset:\n{accuracy_df}\n"
                + "=" * 80
            )
            plot_accuracies(
                accuracy_df,
                f"Accuracy of Letters in {name} Dataset",
                output_dir / f"{name}_letter_accuracy.png",
            )

    # Aggregate accuracies across datasets
    aggregated_accuracies = aggregate_accuracies(accuracy_dict)
    logger.info(
        "Aggregated letter accuracies across datasets:\n%s",
        aggregated_accuracies,
    )
